<template>
    <basic-container>
        <avue-crud
            ...
            <template slot-scope="{disabled,size}" slot="loginTypeSearch">
                <el-select clearable v-model="search.loginType" placeholder="请选择">
                    <el-option
...
<script>
import { getLoginLogList } from "@/api/userApi";
import { getDictListToType } from "@/api/systemApi";
...
export default {
  ...
  methods: {
    ...
    query(params, done) {
      ...
      // 向后端发起请求，获取数据
      getLoginLogList(queryData).then(result => {
        // 请求数据成功,设置表格数据和分页
